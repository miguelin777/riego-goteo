...
const TABS: { id: Tab; label: string }[] = [
  { id: 'diagram',    label: 'Esquema Técnico' },
  { id: 'simulation', label: 'Simulación' },
  { id: 'calcs',      label: 'Pasos de Cálculo' },
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {results.warnings.length > 0 && (
        <div className="max-w-[1520px] mx-auto px-4 mt-3 no-print">
          <AlertsPanel warnings={results.warnings} />
        </div>
      )}

      {/* Main */}
      <main className="max-w-[1520px] mx-auto px-4 py-4">
        <div className="flex gap-5 items-start">
          {/* Columna izquierda: parámetros */}
          <aside className="w-[310px] shrink-0 no-print">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4
                            sticky top-3 max-h-[calc(100vh-5rem)] overflow-y-auto">
          ...
          <section className="flex-1 min-w-0 space-y-4">
            {/* Tabs */}
            <nav className="flex gap-1 flex-wrap no-print">
              {TABS.map(t => (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
              {activeTab === 'diagram' && (
                <div className="space-y-4">
                  <h2 className="text-base font-bold text-slate-800">Esquema Técnico del Sistema</h2>
